...
- **Styling**: Tailwind CSS v4, configured via `@theme` in `index.css` (no `tailwind.config.js` — v4-style CSS-first config). All colors are theme tokens, sourced from `docs/DESIGN.md` — never raw hex/Tailwind palette classes in components. Shared visual primitives are CSS utility classes in `index.css` rather than React components.
