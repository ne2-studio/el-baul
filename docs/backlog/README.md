...
* `./scripts/gap-scout` — runs the `architecture-gap-scout` skill periodically over `app/` or
  `api/` via the `gap-scout-runner` skill, dedupes against previously filed initiatives, and
  files new ones as GitHub issues (label `gap-scout`) pending approval. See
