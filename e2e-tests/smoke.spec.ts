...
test('user can log in with Google (fake-oidc) and reach the El Baúl home screen', async ({ page }) => {
  ...
  // exchanging the code for a token, which raced the localStorage read below. A genuinely
  // fresh fake-oidc admin user has zero baúles and has never seen onboarding, so lands on
  // /onboarding first; a returning one that's already seen it lands on /baules/nuevo.
  ...
  // deterministically exercises the actual home screen, not the onboarding/create-baúl flow.
