...
- **`npm run test:image-acceptance`** (`e2e-image-acceptance/`) — the suite to reach for while
  ...
  (everything in memory, no Postgres/MinIO/imgproxy). What gates `frontend-deploy.yml`. Needs
  both images built first:
  ```bash
...
stack) is not part of this package — see [`/e2e-tests`](../e2e-tests) at the repo root
(`npm run test:e2e` there), ~5x slower than `test:image-acceptance` since it boots real
