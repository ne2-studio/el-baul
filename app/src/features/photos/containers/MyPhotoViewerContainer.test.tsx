...
describe('MyPhotoViewerContainer', () => {
  ...
  // GitHub issue #80: reached from "Sin compartir", the photo is no longer unshared once added
  // to a baúl, so it drops out of useMyPhotosStore (see addBaulAppearance) — the viewer must
  // close/navigate away instead of trying to keep showing a photo it can no longer find.
  it('closes the viewer after adding to a baúl when reached from "Sin compartir"', async () => {
    useMyPhotosStore.getState().setFilter('sin-compartir');
    useBaulesStore.setState({
      baules: [baul('b1', 'Familia Pardal'), baul('b2', 'Familia Jimena')],
    });
    vi.mocked(addPhotoAssetToBaul).mockResolvedValue({ baulId: 'b2', baulName: 'Familia Jimena' });
