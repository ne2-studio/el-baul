...
/// when it's present, active, and still in this baúl; otherwise there is no avatar. Crop/zoom
/// always rides along. Kept in one place so PersonaDtoProjector, AuthorInfoProjector, and
/// PhotoPersonaTagManager don't each re-derive the rule — see commit 096b464, which had to patch
/// all three by hand to add crop.
/// </summary>
public static class PersonaAvatarUrlResolver
{
    public static async Task<string?> ResolveAsync(Persona persona, IPhotoRepository? photoRepository, IPhotoStorage photoStorage)
    {
        ...
        // without one) is treated the same as "no photo-backed avatar", same as the
        // ResolveAsync(Persona, Photo?, ...) overload does for a persona whose AvatarPhotoId is
        // unset.
