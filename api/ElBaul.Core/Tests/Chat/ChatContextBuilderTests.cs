...
public class ChatContextBuilderTests
{
    ...
    public async Task BuildAsync_ShouldTagARecuerdo_WithItsPhotosDate_NotItsWriteDate()
    {
        ...
        var photo = PhotoMother.Create(new PhotoId(Guid.NewGuid()), null, new BaulId(baulId), "key", PhotoDates.Of(1998, 6, 15), new UserId(CustodioId), _clock.UtcNow());
    ...
    public async Task BuildAsync_ShouldTagARecuerdo_WithItsChaptersEarliestPhotoDate_WhenNotDirectlyLinkedToADatedPhoto()
    {
        ...
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(Guid.NewGuid()), chapter.Id, new BaulId(baulId), "key-1", PhotoDates.Of(2010, 9), new UserId(CustodioId), _clock.UtcNow()));
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(Guid.NewGuid()), chapter.Id, new BaulId(baulId), "key-2", PhotoDates.Of(2010, 5), new UserId(CustodioId), _clock.UtcNow()));
    ...
    public async Task BuildAsync_ShouldResolveEachChaptersDateRange_FromItsOwnPhotos()
    {
        ...
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(Guid.NewGuid()), firstChapter.Id, new BaulId(baulId), "key-old", PhotoDates.Of(2005, 1), new UserId(CustodioId), _clock.UtcNow()));
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(Guid.NewGuid()), secondChapter.Id, new BaulId(baulId), "key-new", PhotoDates.Of(2022, 6), new UserId(CustodioId), _clock.UtcNow()));
    ...
    public async Task BuildAsync_ShouldShowASingleDate_InTheChapterHeader_WhenAllItsPhotosAgree()
    {
        ...
        await _chapterRepository.CreateAsync(chapter);
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(Guid.NewGuid()), chapter.Id, new BaulId(baulId), "key-1", PhotoDates.Of(2015, 3, 20), new UserId(CustodioId), _clock.UtcNow()));
    ...
    public async Task BuildSummaryAsync_ShouldIncludeTheChaptersDateRange()
    {
        ...
        await _chapterRepository.CreateAsync(chapter);
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(Guid.NewGuid()), chapter.Id, new BaulId(baulId), "key-1", PhotoDates.Of(2010, 5), new UserId(CustodioId), _clock.UtcNow()));
