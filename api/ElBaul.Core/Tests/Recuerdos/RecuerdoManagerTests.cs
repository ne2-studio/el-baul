...
public class RecuerdoManagerTests
{
    ...
    private RecuerdoManager CreateManager(string currentUserId, Guid? nextId = null) =>
        new(NullLogger<RecuerdoManager>.Instance, _chapterRepository, _photoRepository, _recuerdoRepository,
            ...
            new AuthorInfoProjector(_personaRepository, _photoRepository, _photoStorage), new FakeUnitOfWork());
    ...
    private async Task<(Guid baulId, Guid chapterId)> SeedBaulWithChapterAsync()
    {
        ...
        await _personaRepository.AddPersonaAsync(new Persona(new PersonaId(Guid.NewGuid()), new BaulId(baulId), new UserId(CustodioId), "Custodio", BaulRole.Administrador, _clock.UtcNow()));
        await _chapterRepository.CreateAsync(new Chapter(new ChapterId(chapterId), new BaulId(baulId), "Chapter", 0, _clock.UtcNow(), _clock.UtcNow()));
    ...
    public async Task CreateRecuerdoAsync_ShouldStampIsOwn_ForTheAuthor()
    {
        ...
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(photoId), new ChapterId(chapterId), new BaulId(baulId), "key", null, new UserId(CustodioId), _clock.UtcNow()));
    ...
    public async Task CreateRecuerdoAsync_ShouldIncludeAuthorsAvatarUrl_WhenPersonaHasOne()
    {
        ...
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(photoId), new ChapterId(chapterId), new BaulId(baulId), "key", null, new UserId(CustodioId), _clock.UtcNow()));
        const string colaboradorId = "colaborador-1";
        var avatarPhoto = PhotoMother.Create(new PhotoId(Guid.NewGuid()), null, new BaulId(baulId), "avatar-key", null, new UserId(CustodioId), _clock.UtcNow());
    ...
    public async Task CreateRecuerdoAsync_ShouldLeaveChapterIdNull_ForLoosePhoto()
    {
        ...
        await _baulRepository.CreateAsync(new Baul(new BaulId(baulId), "Familia", null, new UserId(CustodioId), 0, _clock.UtcNow(), _clock.UtcNow()));
        var photoId = Guid.NewGuid();
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(photoId), null, new BaulId(baulId), "key", null, new UserId(CustodioId), _clock.UtcNow()));
    ...
    public async Task CreateRecuerdoAsync_ShouldAllow_ForColaboradorRole()
    {
        ...
        const string colaboradorId = "colaborador-1";
        await _baulRepository.CreateAsync(new Baul(new BaulId(baulId), "Familia", null, new UserId(CustodioId), 0, _clock.UtcNow(), _clock.UtcNow()));
        await _personaRepository.AddPersonaAsync(new Persona(new PersonaId(Guid.NewGuid()), new BaulId(baulId), new UserId(colaboradorId), "Colaborador", BaulRole.Colaborador, _clock.UtcNow()));
        await _chapterRepository.CreateAsync(new Chapter(new ChapterId(chapterId), new BaulId(baulId), "Chapter", 0, _clock.UtcNow(), _clock.UtcNow()));
    ...
    public async Task GetRecuerdosAsync_ShouldResolveEachRecuerdosThumbnail_FromItsOwnPhoto()
    {
        ...
        await _baulRepository.CreateAsync(new Baul(new BaulId(baulId), "Familia", null, new UserId(CustodioId), 0, _clock.UtcNow(), _clock.UtcNow()));
        await _chapterRepository.CreateAsync(new Chapter(new ChapterId(chapterId), new BaulId(baulId), "Chapter", 2, _clock.UtcNow(), _clock.UtcNow()));
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(firstPhotoId), new ChapterId(chapterId), new BaulId(baulId), "first-key", null, new UserId(CustodioId), _clock.UtcNow()));
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(secondPhotoId), new ChapterId(chapterId), new BaulId(baulId), "second-key", null, new UserId(CustodioId), _clock.UtcNow()));
    ...
    public async Task GetRecuerdosAsync_ShouldReturnMixedFeed_NewestFirst_WithProvenance()
    {
        ...
        var photoId = Guid.NewGuid();
        await SeedBaulAsync(baulId, "Familia");
        await _chapterRepository.CreateAsync(new Chapter(new ChapterId(chapterId), new BaulId(baulId), "Vacaciones", 1, _clock.UtcNow(), _clock.UtcNow()));
        await _photoRepository.CreateAsync(PhotoMother.Create(new PhotoId(photoId), new ChapterId(chapterId), new BaulId(baulId), "photo-key", null, new UserId(CustodioId), _clock.UtcNow()));
    ...
    public async Task GetRecuerdosAsync_ShouldResolveEachRecuerdosChapterName_FromItsOwnChapter()
    {
        ...
        await SeedBaulAsync(baulId, "Familia");
        await _chapterRepository.CreateAsync(new Chapter(new ChapterId(firstChapterId), new BaulId(baulId), "Capítulo uno", 0, _clock.UtcNow(), _clock.UtcNow()));
        await _chapterRepository.CreateAsync(new Chapter(new ChapterId(secondChapterId), new BaulId(baulId), "Capítulo dos", 0, _clock.UtcNow(), _clock.UtcNow()));
