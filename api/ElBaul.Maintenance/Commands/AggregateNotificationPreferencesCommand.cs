using System.Globalization;
using ElBaul.Core.Analytics.OutputPorts;
using Microsoft.Extensions.Logging;

namespace ElBaul.Maintenance.Commands;

/// <summary>
/// Rebuilds analytics.notification_preferences_daily for an inclusive date range. Safe to
/// re-run: each date delegates to the same idempotent day-level aggregator used by Hangfire.
/// </summary>
[MaintenanceCommand("aggregate-notification-preferences")]
public class AggregateNotificationPreferencesCommand(
    INotificationPreferencesDailyAggregator aggregator,
    MaintenanceCommandArguments arguments,
    ILogger<AggregateNotificationPreferencesCommand> logger) : IMaintenanceCommand
{
    public async Task<int> RunAsync(bool dryRun)
    {
        if (!TryGetDate("--from", out var from) || !TryGetDate("--to", out var to))
        {
            logger.LogError("Usage: aggregate-notification-preferences --from yyyy-MM-dd --to yyyy-MM-dd");
            return 1;
        }

        if (from > to)
        {
            logger.LogError("--from must be earlier than or equal to --to");
            return 1;
        }

        var count = to.DayNumber - from.DayNumber + 1;
        logger.LogInformation(
            "aggregate-notification-preferences: {Count} day(s), from {From} to {To}{DryRunSuffix}",
            count, from, to, dryRun ? " (dry run — no changes will be saved)" : "");

        for (var date = from; date <= to; date = date.AddDays(1))
        {
            if (dryRun)
            {
                logger.LogInformation("Would aggregate notification preferences for {Date}", date);
                continue;
            }

            await aggregator.AggregateForDateAsync(date);
        }

        return 0;
    }

    private bool TryGetDate(string name, out DateOnly date)
    {
        date = default;
        var index = arguments.Values.ToList().IndexOf(name);
        return index >= 0
            && index + 1 < arguments.Values.Count
            && DateOnly.TryParseExact(arguments.Values[index + 1], "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out date);
    }
}
