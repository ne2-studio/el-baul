...
/// <summary>
/// Rebuilds analytics.user_activity_daily for an inclusive date range. Safe to re-run:
    ...
    ILogger<AggregateUserActivityCommand> logger) : IMaintenanceCommand
{
    public async Task<int> RunAsync(bool dryRun)
    {
        ...
        {
            logger.LogError("Usage: aggregate-user-activity --from yyyy-MM-dd --to yyyy-MM-dd");
            return 1;
        }
        ...
        logger.LogInformation(
            "aggregate-user-activity: {Count} day(s), from {From} to {To}{DryRunSuffix}",
