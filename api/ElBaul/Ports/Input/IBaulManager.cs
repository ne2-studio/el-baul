...
public interface IBaulManager
{
    ...
    Task<Result<IEnumerable<AccessRequestDto>>> GetAccessRequestsAsync(Guid baulId);
    Task<Result<AccessRequestDto>> CreateAccessRequestAsync(Guid baulId, string? message);
    Task<Result<SharedUserDto>> ApproveAccessRequestAsync(Guid baulId, Guid requestId, string role);
    Task<Result> RejectAccessRequestAsync(Guid baulId, Guid requestId);
